    ...
    def handle(self):
        ...
        if command == 'status':
            ...
            if not self.server.runners:
                self.request.sendall('no runners are registerd yet')
            else:
                self.request.sendall('OK')
            ...
            remaining_buffer = self.BUFFER_SIZE - (len(command) + len(results[1]) + 3)
...
def dispatch_into_tests(server, commit_hash):
    while True:
        ...
        for runner in server.runners:
            response = helpers.communicate(runner['host'], int(runner['port']), 'runtest: %s' % commit_hash)
            if response == 'OK':
...
def serve():
    ...
    parser.add_argument('--host', help='forwarder\' host. localhost by default', default='localhost', action='store')
    parser.add_argument('--port', help='forwarder\' port. 8878 by default', default=8878, action='store')
    ...
    def test_checker(server):
        def manage_commit_lists(runner):
            for commit, assigned_runner in server.forwarded_commits.ineritems():
